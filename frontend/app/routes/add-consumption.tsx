  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `/api/product/search?name=${encodeURIComponent(searchTerm)}`
      );
      ...
      <div className="max-w-2xl mx-auto pb-20">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-6">
          <form onSubmit={handleSearch} className="relative">
            ...
            <button
              type="submit"
              className="absolute right-2 top-2 bg-emerald-600 text-white p-1.5 rounded-lg hover:bg-emerald-700 cursor-pointer"
            >
              {loading ? (
        ...
        {selectedProduct && (
          <div className="bg-white border border-slate-200 p-6 rounded-2xl mb-6 shadow-sm animate-in fade-in slide-in-from-bottom-4">
              ...
              <div>
                ...
                <div className="flex gap-3 mt-2 text-xs font-medium">
                  <span className="text-pink-600 bg-pink-50 px-2 py-0.5 rounded">
                    Sucre: {selectedProduct.sugars}g/100ml
                  </span>
                  <span className="text-purple-600 bg-purple-50 px-2 py-0.5 rounded">
                    Caféine: {selectedProduct.caffeine}mg/100ml
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                <label className="flex items-center gap-2 text-sm font-medium text-slate-700 mb-1.5">
                  <MapPin className="w-4 h-4 text-slate-400" /> Lieu{" "}
