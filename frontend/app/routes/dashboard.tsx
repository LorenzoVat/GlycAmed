...
interface DashboardData {
  totals: { sugar: number; caffeine: number; calories: number; contributions: number; };
...
export default function Dashboard() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-emerald-600 font-medium">Chargement...</div>;
  ...
  const caffeineWidth = Math.min((caffeineCurrent / LIMITS.CAFFEINE) * 100, 100);
  ...
  if (isSugarOver && isCaffeineOver) {
    ...
    statusMessage = "Alerte rouge ! Sucre et caféine sont au-dessus des seuils.";
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-24">
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-10 px-4 lg:px-8">
        <div className="max-w-7xl mx-auto h-16 flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
                <div className="hidden md:flex items-center gap-2 text-slate-600 text-sm bg-slate-100 px-3 py-1.5 rounded-full">
                    <UserIcon className="w-4 h-4 text-emerald-700" />
                    <span className="font-medium">{user?.firstName} {user?.lastName}</span>
                </div>
                <button onClick={handleLogout} className="text-slate-500 hover:text-red-600 transition-colors flex items-center gap-2 text-sm font-medium">
      ...
      <main className="max-w-7xl mx-auto px-4 lg:px-8 py-8 space-y-8">
        ...
        <div className={clsx("rounded-2xl p-6 border flex items-start gap-4 shadow-sm", statusColor)}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm col-span-1 md:col-span-2 lg:col-span-1">
                <div className="flex items-center justify-between mb-4">
                    ...
                    {isSugarOver && <span className="bg-red-100 text-red-700 text-xs px-2 py-1 rounded-full font-bold animate-pulse">DANGER</span>}
                </div>
                <div className="flex items-baseline gap-1 mb-2">
                    <span className={clsx("text-3xl font-bold", isSugarOver ? "text-red-600" : "text-slate-900")}>{sugarCurrent.toFixed(1)}</span>
                    <span className="text-slate-400 text-sm">/ {LIMITS.SUGAR}g</span>
                </div>
                <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
                    <div className={clsx("h-full rounded-full transition-all duration-700 ease-out", getBarColor(sugarCurrent, LIMITS.SUGAR))} style={{ width: `${sugarWidth}%` }}></div>
                </div>
                <p className="text-xs text-slate-400 mt-2 text-right">Max recommandé : {LIMITS.SUGAR}g/jour</p>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm col-span-1 md:col-span-2 lg:col-span-1">
                 <div className="flex items-center justify-between mb-4">
                    ...
                    {isCaffeineOver && <span className="bg-red-100 text-red-700 text-xs px-2 py-1 rounded-full font-bold animate-pulse">DANGER</span>}
                </div>
                <div className="flex items-baseline gap-1 mb-2">
                    <span className={clsx("text-3xl font-bold", isCaffeineOver ? "text-red-600" : "text-slate-900")}>{caffeineCurrent.toFixed(0)}</span>
                    <span className="text-slate-400 text-sm">/ {LIMITS.CAFFEINE}mg</span>
                </div>
                <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
                     <div className={clsx("h-full rounded-full transition-all duration-700 ease-out", getBarColor(caffeineCurrent, LIMITS.CAFFEINE))} style={{ width: `${caffeineWidth}%` }}></div>
                </div>
                <p className="text-xs text-slate-400 mt-2 text-right">Max recommandé : {LIMITS.CAFFEINE}mg/jour</p>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                ...
                <span className="text-3xl font-bold block text-slate-900">{stats?.totals.calories.toFixed(0)}</span>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                ...
                <span className="text-3xl font-bold block text-slate-900">{stats?.totals.contributions}</span>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            {history.length === 0 ? (
                <div className="p-12 text-center text-slate-500">Aucune activité récente.</div>
            ) : (
                <ul className="divide-y divide-slate-50">
                    {history.slice(0, 5).map((item) => (
                        <li key={item._id} className="p-4 hover:bg-slate-50 transition-colors">
                            <div className="flex items-start gap-4">
                                ...
                                <div className="text-right shrink-0">
                                    <span className={clsx(
                                        ...
                                        item.nutrients.sugar > 20 ? "bg-red-100 text-red-800" : "bg-slate-100 text-slate-800"
                                    )}>
