...
import { ArrowLeft, Search, Calendar, MapPin, User, X, Filter } from "lucide-react";
...
export default function History() {
  ...
  const filteredConsumptions = consumptions.filter(item => {
      ...
      const fullName = `${item.contributorId?.firstName} ${item.contributorId?.lastName}`.toLowerCase();
  ...
  const hasFilters = productFilter || dateFilter || locationFilter || contributorFilter;
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {loading ? (
                            ...
                            <tr><td colSpan={6} className="p-8 text-center text-slate-500">Aucun résultat ne correspond à vos filtres.</td></tr>
                        ) : (
                            filteredConsumptions.map((item) => (
                                <tr key={item._id} className="hover:bg-slate-50 transition-colors">
                                    <td className="p-4 text-slate-500 text-sm whitespace-nowrap">
                                        ...
                                        <span className="text-xs opacity-70">{new Date(item.consumedAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                                    </td>
                                    <td className="p-4">
                                        <span className="font-medium text-emerald-900 block">{item.productName}</span>
                                    </td>
                                    ...
                                    <td className="p-4 text-right text-slate-600">{item.quantityMl} ml</td>
                                    <td className="p-4 text-right font-medium">
                                        {item.nutrients.sugar > 20 ? (
                                            <span className="text-red-600">{item.nutrients.sugar}g</span>
                                        ) : (
                                            <span className="text-slate-700">{item.nutrients.sugar}g</span>
                                        )}
                                    </td>
                                    <td className="p-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-sm text-slate-700">{item.contributorId?.firstName}</span>
                                        </div>
                                    </td>
                                </tr>
                            ))
